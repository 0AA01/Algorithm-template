...
struct P
{
    ...
    P rot90() { return P(-y, x); } // 逆时针90 
    P unit() { return *this / abs(); }// 单位向量 
    int quad() const { return sign(y) == 1 || (sign(y) == 0 && sign(x) >= 0); }
    //判断点在极角坐标系上半边还是下半边，极点和极轴也算上半边
    P rot(db an) { return {x * cos(an) - y * sin(an), x * sin(an) + y * cos(an)}; } // 顺时针an度 
    //向量旋转
};
//线类，半平面类
struct L
...
// 点p是不是在线段p1p2上 (精度容易出错)
bool onSeg(P p1, P p2, P q)
{
    return crossOp(p1, p2, q) == 0 && isMiddle(p1, q, p2);
}
// 点 p 是不是在严格线段 p1p2 上(不包含端点) (精度容易出错)
bool onSeg_strict(P p1, P p2, P q)
{
    return crossOp(p1, p2, q) == 0 && sign((q - p1).dot(p1 - p2)) * sign((q - p2).dot(p1 - p2)) < 0;
}

// 求 q 到p1p2的投影 （垂足） （p1 不可以等于 p2） 
P proj(P p1, P p2, P q)
{
    P dir = p2 - p1;
    return p1 + dir * (dir.dot(q - p1) / dir.abs2());
}
//  求 q 以 直线p1p2为轴的反射  （p1 不可以等于 p2） 
...
int contain(vector<P> ps, P p) //判断点在多边形内
...
db convexDiameter(vector<P> ps) //凸包直径
{
    int n = ps.size();
    if (n <= 1)
        return 0;
...
vector<P> convexCut(const vector<P> &ps, P q1, P q2) //直线切割凸包，返回直线左边凸包的点
{
    vector<P> qs;
    int n = ps.size();
    for (int i = 0; i < n; i++)
